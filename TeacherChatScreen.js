...
export default function TeacherChatScreen() {
  ...
  // console.log("the current user is",currentUser1);
// const currentUser = "UJIVyomg3QRgNR9QbgfehjS3ueH2";
...
useEffect(() => {
      ...
      onValue(dbRef, (snapshot) => {
        ...
        if (userData) {
          ...
          const filteredMessages = messagesArray.filter(
            (message) => message.senderUid === teacherUid || message.receiverUid === teacherUid
          );
  ...
  const renderMessage = ({ item }) => {
    ...
    return (
      <View style={isCurrentUserSender ? styles.receiverMessage : styles.senderMessage}>
